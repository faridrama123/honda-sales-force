...
class GeneralKnowledgeFragment : Fragment(), BaseFragmentInterface {
...
//
                ...
                if(f.complaint_handling_detail!=null && f.complaint_handling_detail.size>0){
                    for(f1 in f.complaint_handling_detail){
                        addText(f1.body,f1.text_align,llspeccontainer)
                        addImage(f1.image,f1.image_align,llspeccontainer)
                    }
                }
    ...
    companion object {
        fun Instantiate(tit: String, ls: List<productfeature>): GeneralKnowledgeFragment {
            val sd = GeneralKnowledgeFragment()
            sd.title = tit
            sd.lsprofit = ls
            return sd
        }
