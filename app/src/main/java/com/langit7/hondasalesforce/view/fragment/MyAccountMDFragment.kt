...
class MyAccountMDFragment : Fragment(), BaseFragmentInterface {
...
//            ttakesurvey.setText(usr!!.total_survey)
        ...
        var scroller=rootView.findViewById<ScrollView>(R.id.scrollview)
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            scroller.setOnScrollChangeListener{ v, i, scrollY, i3, i4 ->
                var s=v as ScrollView
//                Log.e("y",scrollY.toString())
                if (s.getChildAt(0).getBottom()
                    <= (s.getHeight() + s.getScrollY())) {
//                    Log.e("BOTTOM",scrollY.toString())


                    (lsFragment.get(1) as LeaderboardListMDFragment).renderLead(lsMD)
                    view_pager.onPageChanged((lsFragment.get(1) as LeaderboardListMDFragment).view)
    ...
    companion object {
        fun Instantiate(lsmd:ArrayList<leaderboard>): MyAccountMDFragment {
            val sd = MyAccountMDFragment()
            sd.lsMD=lsmd
            return sd
        }
    }
...
//        lsFragment.add(QuisHistoryFragment.Instantiate(lsHQ))
        lsFragment.add(LeaderboardListMDFragment.InstantiateMD(lsMD,"TOP 10 Sales Force di area Main Dealer","",false))
